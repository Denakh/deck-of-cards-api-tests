...
import com.github.denakh.deckofcardsapi.model.Card;
import com.github.denakh.deckofcardsapi.model.DeckInfo;
import com.github.denakh.deckofcardsapi.model.Pile;
...
public class DeckOfCardsTest {
    ...
    public void pileCardsShouldCorrespondToUsedForPilesBuilding() {
        ...
        assertEquals(pilesInfoWithPile1Cards.getPiles().getOrDefault(pile1Name, new Pile()).getCards().stream().sorted().collect(Collectors.toList()),
                firstPileCards.stream().sorted().collect(Collectors.toList()));
        assertEquals(pilesInfoWithPile2Cards.getPiles().getOrDefault(pile2Name, new Pile()).getCards().stream().sorted().collect(Collectors.toList()),
